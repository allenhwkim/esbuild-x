...
function injectWebSocketClient(port, srcIndexHTML, outIndexPath, fs) {
  const wsHtml = `\n<script>setTimeout(_ => {` +
      `var ws = new WebSocket('ws://localhost:${port}');` +
      `ws.onmessage = e => { console.log('ws reload requested'); window.location.reload(); }` +
    `}, 1000)</script>`;
  const contents = srcIndexHTML.replace(/<\/body>/, `${wsHtml}\n</body>`);
  fs.writeFileSync(outIndexPath, contents);
} 

module.exports = function runWatchAndReload(watchDir, websocketPort = 9110, srcIndexPath) {
  ...
  return function runWatchAndReload(options, buildResult) { 
    ...
    const srcIndexHTML = require('fs').readFileSync(srcIndexPath, {encoding: 'utf8'});
    const outIndexPath = path.join(options.outdir, 'index.html');
    injectWebSocketClient(websocketPort, srcIndexHTML, outIndexPath, fs); // this injects ws client into index.html
    ...
    watcher.on('all', async (event, filePath) => {
      ...
      const indexPath = injectEsbuildResult(srcIndexPath)(options, esbuildResult);
      // Inject websocket client to index.html
      injectWebSocketClient(websocketPort, srcIndexHTML, outIndexPath, fs);
